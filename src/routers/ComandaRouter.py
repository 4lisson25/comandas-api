...
async def get_comanda(id: int, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
    try:
        ...
        if not row:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Comanda não encontrada")
...
async def get_comandas(
    ...
    comanda: Optional[int] = Query(
        None, description="Filtrar por número da comanda"),
    status: Optional[int] = Query(
        None, description="Filtrar por status: 0=aberta, 1=fechada, 2=cancelada"),
    ...
    data_inicio: Optional[datetime] = Query(
        None, description="Filtrar por data inicial"),
    data_fim: Optional[datetime] = Query(
        None, description="Filtrar por data final"),
...
async def create_comanda(comanda_data: ComandaCreate, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(get_current_active_user)):
    try:
        ...
        if not funcionario:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Funcionário não encontrado")
        ...
        if comanda_data.cliente_id:
            ...
            if not cliente:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="Cliente não encontrado")
        ...
        AuditoriaService.registrar_acao(db=db, funcionario_id=current_user.id, acao="CREATE",
                                        recurso="COMANDA", recurso_id=new_comanda.id, dados_novos=new_comanda, request=request)
...
async def cancelar_comanda(
    ...
    try:
        ...
        if comanda.status == 2:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Comanda já está cancelada")
        ...
        AuditoriaService.registrar_acao(db=db, funcionario_id=current_user.id, acao="CANCEL",
                                        recurso="comanda", recurso_id=comanda.id, dados_antigos=dados_antigos, request=request)
...
async def add_produto_to_comanda(comanda_id: int, produto_data: ComandaProdutosCreate, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth =
    ...
    try:
        ...
        if not produto:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Produto não encontrado")
...
async def remove_produto_from_comanda(id: int, request: Request, db: AsyncSession = Depends(get_async_db), current_user: FuncionarioAuth = Depends(require_group([1]))):
    try:
        ...
        # Registrar auditoria de exclusão
        AuditoriaService.registrar_acao(db=db, funcionario_id=current_user.id, acao="DELETE",
                                        recurso="COMANDA_PRODUTO", recurso_id=id, dados_antigos=comanda_produto, request=request)
